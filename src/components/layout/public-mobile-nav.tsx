        ...
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <aside className="absolute right-0 top-0 flex h-full w-[min(100%,320px)] flex-col border-l bg-card shadow-2xl animate-slide-up safe-top safe-bottom">
            <div className="flex h-14 items-center justify-between border-b px-4 sm:h-16">
              <BrandLogo href="/" size="sm" context="organization" showText={false} prominent />
            ...
            <div className="space-y-2 border-t p-4">
              <Button variant="outline" className="w-full" asChild>
                <Link href={landingConfig.signIn.href} onClick={() => setOpen(false)}>
                  {landingConfig.signIn.label}
                </Link>
              </Button>
              <Button
                className="w-full bg-[hsl(213,52%,25%)] dark:bg-cyan-600"
                asChild
              >
                <Link href={landingConfig.primaryCta.href} onClick={() => setOpen(false)}>
