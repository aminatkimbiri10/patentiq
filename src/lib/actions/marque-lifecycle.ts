...
export type MarqueLifecycleActionState = { success?: boolean; error?: string; message?: string };

export async function updateMarqueLifecycle(
  _prev: MarqueLifecycleActionState,
  formData: FormData
): Promise<MarqueLifecycleActionState> {
  ...
  const prev = (meta.marque_lifecycle as Record<string, unknown>) ?? {};
  const status = parsed.data.status as MarqueLifecycleStatus;
  ...
  if (status === "publie" && !prev.published_at) {
    ...
    lifecycle.opposition_deadline_at = computeOppositionDeadline(now);
  }
  ...
  const { error } = await supabase
    .from("projects")
    .update({
      metadata: { ...meta, marque_lifecycle: lifecycle },
      updated_at: now,
    })
    .eq("id", parsed.data.project_id);
  ...
  return { success: true, message: "Cycle marque mis à jour." };
}
