...
export type BrevetLifecycleActionState = { success?: boolean; error?: string; message?: string };

export async function updateBrevetLifecycle(
  _prev: BrevetLifecycleActionState,
  formData: FormData
): Promise<BrevetLifecycleActionState> {
  ...
  const prev = (meta.brevet_lifecycle as Record<string, unknown>) ?? {};
  const status = parsed.data.status as BrevetLifecycleStatus;
  ...
  if (status === "depose" && !prev.deposited_at) {
    ...
    lifecycle.publication_deadline_at = computePublicationDeadline(now);
  }
  if (status === "publie" && !prev.published_at) {
    lifecycle.published_at = now;
  }
  if (status === "accorde" && !prev.granted_at) {
    lifecycle.granted_at = now;
  }

  const { error } = await supabase
    .from("projects")
    .update({
      metadata: { ...meta, brevet_lifecycle: lifecycle },
      updated_at: now,
    })
    .eq("id", parsed.data.project_id);
  ...
  return { success: true, message: "Cycle brevet mis à jour." };
}
